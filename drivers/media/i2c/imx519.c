...
static int imx519_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	try_fmt_img->width = supported_modes_10bit[2].width;
	try_fmt_img->height = supported_modes_10bit[2].height;
...
static long imx519_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_GET_CHANNEL_INFO:
		...
		ret = imx519_get_channel_info(imx519, ch_info);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	mutex_lock(&imx519->mutex);
	fi->interval = imx519->mode->timeperframe_default;
	mutex_unlock(&imx519->mutex);
...
static int imx519_init_controls(struct imx519 *imx519)
{
	...
	imx519->vblank = v4l2_ctrl_new_std(ctrl_hdlr, &imx519_ctrl_ops,
					   V4L2_CID_VBLANK, 0, 0xffff, 1, 251);
	imx519->hblank = v4l2_ctrl_new_std(ctrl_hdlr, &imx519_ctrl_ops,
					   V4L2_CID_HBLANK, 0, 0xffff, 1, 7994);
...
MODULE_DEVICE_TABLE(of, imx519_match_id);
#endif
...

{
	...
	imx519->mode = &supported_modes_10bit[2];

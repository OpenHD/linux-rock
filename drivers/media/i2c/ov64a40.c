...
struct ov64a40_mode {
    ...
    struct v4l2_fract fps_default[OV64A40_NUM_LINK_FREQ];
...
static struct ov64a40_mode supported_modes[] = {
	/* Full resolution */
	{
		.width = 9248,
		.height = 6944,
...
static inline struct ov64a40 *to_ov64a40(struct v4l2_subdev *sd)
{
...
{
	struct ov64a40 *ov64a40 = to_ov64a40(sd);
	int ret;

	if (enable)
		ret = ov64a40_start_streaming(ov64a40);
	else
		ret = ov64a40_stop_streaming(ov64a40);

	return ret;
}

static int ov64a40_g_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct ov64a40 *ov64a40 = to_ov64a40(sd);
	const struct ov64a40_mode *mode = ov64a40->mode;
    s64 link_freq = ov64a40->link_frequencies[ov64a40->link_freq->cur.val];
	unsigned int timings_index = link_freq == OV64A40_LINK_FREQ_360M
				   ? OV64A40_LINK_FREQ_360M_ID
				   : OV64A40_LINK_FREQ_456M_ID;
	fi->interval = mode->fps_default[timings_index];
...
static const struct v4l2_subdev_video_ops ov64a40_video_ops = {
    ...
    .g_frame_interval = ov64a40_g_frame_interval,
};
			    ...
			    struct v4l2_subdev_pad_config *cfg)
{
	...
	ov64a40_update_pad_fmt(ov64a40, &supported_modes[0], format);
				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	mode = &supported_modes[fse->index];
...
static int ov64a40_get_format(struct v4l2_subdev *sd,
			      struct v4l2_subdev_pad_config *cfg,
			      struct v4l2_subdev_format *fmt)
{
	...
	// mutex_lock(&ov64a40->mutex);
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
#ifdef CONFIG_VIDEO_V4L2_SUBDEV_API
		fmt->format = *v4l2_subdev_get_try_format(sd, cfg, fmt->pad);
#else
		mutex_unlock(&ov64a40->mutex);
		return -ENOTTY;
#endif
	} else {
		fmt->format.width = mode->width;
		fmt->format.height = mode->height;
		fmt->format.code = ov64a40_mbus_code(ov64a40);
			      ...
			      struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_ACTIVE) {
		...
		ov64a40->mode = mode;
		*v4l2_subdev_get_try_crop(sd, cfg, 0) = mode->analogue_crop;
...
static const struct v4l2_subdev_pad_ops ov64a40_pad_ops = {
	...
	.get_fmt = ov64a40_get_format,
	.set_fmt = ov64a40_set_format,
	//.get_selection = ov64a40_get_selection,
};


static void ov64a40_get_module_inf(struct ov64a40 *ov64a40,
				  struct rkmodule_inf *inf)
{
	// struct i2c_client *client = v4l2_get_subdevdata(&ov64a40->sd);

	memset(inf, 0, sizeof(*inf));
	strlcpy(inf->base.sensor, OV64A40_NAME, sizeof(inf->base.sensor));
	strlcpy(inf->base.module, ov64a40->module_name,
		sizeof(inf->base.module));
	strlcpy(inf->base.lens, ov64a40->len_name, sizeof(inf->base.lens));

	// v4l2_dbg(1, debug, ov64a40->sd,"%s: get_module_inf:%s, %s, %s.\n", __func__,
	// 	inf->base.sensor, inf->base.module, inf->base.lens);
}

static int ov64a40_get_channel_info(struct ov64a40 *ov64a40, struct rkmodule_channel_info *ch_info)
{
	if (ch_info->index < PAD0 || ch_info->index >= PAD_MAX)
		return -EINVAL;
	ch_info->width = ov64a40->mode->width;
	ch_info->height = ov64a40->mode->height;
	ch_info->bus_fmt = ov64a40_mbus_code(ov64a40); 
	ch_info->vc = V4L2_MBUS_CSI2_CHANNEL_0;

	return 0;
}

static long ov64a40_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct ov64a40 *ov64a40 = to_ov64a40(sd);
	...
	case RKMODULE_GET_MODULE_INFO:
		ov64a40_get_module_inf(ov64a40, (struct rkmodule_inf *)arg);
		break;
	case RKMODULE_GET_CHANNEL_INFO:
		ch_info = (struct rkmodule_channel_info *)arg;
		ret = ov64a40_get_channel_info(ov64a40, ch_info);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
	...
	// printk("%s: cmd:[%u]  ret: %ld",__func__, cmd, ret);

	return ret;
}

#ifdef CONFIG_COMPAT
static long ov64a40_compat_ioctl32(struct v4l2_subdev *sd,
				  unsigned int cmd, unsigned long arg)
{
	...
	case RKMODULE_GET_CHANNEL_INFO:
		...
		ret = ov64a40_ioctl(sd, cmd, ch_info);
		if (!ret) {
...
static const struct v4l2_subdev_core_ops ov64a40_core_ops = {
    ...
    .ioctl	= ov64a40_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = ov64a40_compat_ioctl32,
#endif
};
...
static int ov64a40_parse_dt(struct ov64a40 *ov64a40)
{
	...
	for (i = 0; i < v4l2_fwnode.nr_of_link_frequencies; ++i) {
		...
		printk("ov64a40->link_frequencies[%d]: %lld\r\n",i, ov64a40->link_frequencies[i] );
	}
...
#ifdef CONFIG_VIDEO_V4L2_SUBDEV_API
static int ov64a40_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct ov64a40 *ov64a40 = to_ov64a40(sd);
	struct v4l2_mbus_framefmt *try_fmt =
				v4l2_subdev_get_try_format(sd, fh->pad, 0);
	const struct ov64a40_mode *def_mode = &supported_modes[0];

	// mutex_lock(&ov64a40->mutex);
	/* Initialize try_fmt */
	try_fmt->width = def_mode->width;
	try_fmt->height = def_mode->height;
	try_fmt->code = ov64a40_mbus_code(ov64a40); 
                    ...
                    const struct i2c_device_id *id)
{
	...
	ret = of_property_read_u32(node, RKMODULE_CAMERA_MODULE_INDEX,
				   &ov64a40->module_index);
	ret |= of_property_read_string(node, RKMODULE_CAMERA_MODULE_FACING,
				       &ov64a40->module_facing);
	...
	ov64a40->mode = &supported_modes[0];
	...
	if (strcmp(ov64a40->module_facing, "back") == 0)
		facing[0] = 'b';
	else
		facing[0] = 'f';

	snprintf(ov64a40->sd.name, sizeof(ov64a40->sd.name), 
		 "m%02d_%s_%s %s", ov64a40->module_index, facing,
		 OV64A40_NAME, dev_name(ov64a40->sd.dev));
	ret = v4l2_async_register_subdev_sensor_common(&ov64a40->sd);
	if (ret < 0) {
		dev_err(&client->dev,
...
static int ov64a40_remove(struct i2c_client *client)
{
...
static const struct of_device_id ov64a40_of_match[] = {
	{ .compatible = "ovti,ov64a40" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, ov64a40_of_match);
#endif

static const struct i2c_device_id ov64a40_match_id[] = {
	{ "ovti,ov64a40", 0 },
	{ },
};
...
static struct i2c_driver ov64a40_i2c_driver = {
	.driver	= {
		.name = OV64A40_NAME,
		.of_match_table	= of_match_ptr(ov64a40_of_match),
		.pm = &ov64a40_pm_ops,
	},
	.probe	= ov64a40_probe,
	.remove	= ov64a40_remove,
    .id_table	= ov64a40_match_id,

};

static int __init sensor_mod_init(void)
{
	return i2c_add_driver(&ov64a40_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&ov64a40_i2c_driver);
}
